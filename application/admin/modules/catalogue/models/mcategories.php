...
class Mcategories extends AG_Model
{
	...
	public function render_categories_grid()
	{
		...
		$this->load->helper('catalogue/categories_helper');
	...
	{
		$data = array();
		$query = $this->db->select("A.`id_parent`, A.`level`, A.`active`, A.`open`, A.`show`, A.`url`, B.*")
				->from("`".self::CAT."` AS A")
				->join(	"`".self::CAT_DESC."` AS B",
			...
			if($this->form_validation->run())
			{
				if($id)
				{
					...
					if($query->num_rows() == 1)
					{
						...
						*/
						$this->db->trans_start();
						$this->sql_add_data($POST)->sql_update_date()->sql_using_user()->sql_save(self::CAT, $id);
				...
				else
				{
					...
					$this->db->trans_start();
					$ID = $this->sql_add_data($POST)->sql_update_date()->sql_using_user()->sql_save(self::CAT);
					if($ID && $ID > 0)
					{
