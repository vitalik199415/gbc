...
class Mproducts_view extends AG_Model
{
	...
	protected function get_product_detail_query($id)
	{
		...
		{
			$query = $this->db
				->select("A.`".self::ID_PR."` AS ID, A.`sku`, A.`url_key`, A.`status`, A.`bestseller`, A.`sale`, A.`new`, B.`name`, B.`full_description`, B.`seo_title`, B.`seo_description`, B.`seo_keywords`,
		...
		$query = $this->db
				->select("A.`".self::ID_PR."` AS ID, A.`sku`, A.`url_key`, A.`status`, A.`in_stock`, A.`bestseller`, A.`sale`, A.`new`, B.`name`, B.`full_description`, B.`seo_title`, B.`seo_description`, B.`seo_keywords`")
				->from("`".self::PR."` AS A")
				->join(	"`".self::PR_DESC."` AS B",
						"B.`".self::ID_PR."` = A.`".self::ID_PR."` && B.`".self::ID_LANGS."` = '".$this->mlangs->id_langs."'",
						"left")
